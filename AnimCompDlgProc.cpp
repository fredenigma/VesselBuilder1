...
void DialogControl::UpdateAnimCompGroupLists(HWND hWnd, def_idx meshdef_idx) {
	...
	for (UINT i = 0; i < VB1->MshMng->GetMeshDefNGrps(meshdef_idx); i++) {
		
		if (VB1->IsUintInVector(i, AnimMng->GetAnimCompDefGroups(idx))) {
			char cbuf[128] = { '\0' };
			sprintf(cbuf, "%i", i);
...
void DialogControl::UpdateAnimCompDialog(HWND hWnd) {
	...
	if (!AnimMng->IsAnimCompArmTip(idx)) {
	...
	for (UINT i = 0; i < VB1->MshMng->GetMeshCount(); i++) {
		SendDlgItemMessage(hWnd, IDC_COMBO_ANIMCOMPMESH, CB_INSERTSTRING, i, (LPARAM)VB1->MshMng->GetMeshDefName(i).c_str());
		SendDlgItemMessage(hWnd, IDC_COMBO_ANIMCOMPMESH, CB_SETITEMDATA, i, (LPARAM)VB1->MshMng->GetMeshIdx(i));
	}
	UINT mdindex = VB1->MshMng->IdxMsh2Def(AnimMng->GetAnimCompDefMesh(idx));
	if (mdindex != (UINT)-1) {
	...
	for (UINT i = 0; i < idx;i++){// AnimMng->GetAnimCompDefCount(); i++) {
		...
		SendDlgItemMessage(hWnd, IDC_COMBO_ANIMCOMPPARENT, CB_SETITEMDATA, index, (LPARAM)AnimMng->GetAnimCompDefACH(i));
	}
	...
	ANIMATIONCOMPONENT_HANDLE pach = AnimMng->GetAnimCompDefParent(idx);
	if (pach!= NULL) {
		int comboindex = ComboFindItemData(GetDlgItem(hWnd, IDC_COMBO_ANIMCOMPPARENT), (void*)pach);
...
BOOL DialogControl::AnimCompDlgProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
	...
	case WM_COMMAND:
	{
		...
		case IDC_BUTTON_ANIMCOMPSETGRPS:
		{
			...
			AnimMng->AddAnimCompDefGroups(idx, grpstopass);
			delete[] sellist;
			
			UpdateAnimCompGroupLists(hWnd, VB1->MshMng->IdxMsh2Def(AnimMng->GetAnimCompDefMesh(idx)));
			AnimMng->SetIsAnimCompArmTip(idx, false);
		...
		case IDC_BUTTON_ANIMCOMPBACKGRPS:
		{
			...
			AnimMng->RemoveAnimCompDefGroups(idx, grpstopass);
		...
		//case IDC_LIST_ANIMCOMPGRPSINANIM:
		{
			...
			case LBN_SELCHANGE:
			{
				VB1->MshMng->ResetMeshGroupHighlights(AnimMng->GetAnimCompDefMesh(idx));
				int nsel = SendDlgItemMessage(hWnd, IDC_LIST_ANIMCOMPGRPSTOPICK, LB_GETSELCOUNT, 0, 0);
				if (nsel > 0) {
					UINT *sellist = new UINT[nsel];
				SendDlgItemMessage(hWnd, IDC_LIST_ANIMCOMPGRPSTOPICK, LB_GETSELITEMS, nsel, (LPARAM)sellist);
				for (UINT i = 0; i < nsel; i++) {
					...
					if (!VB1->MshMng->MeshGroupIsHighlighted(AnimMng->GetAnimCompDefMesh(idx), groupnumber)) {
						VB1->MshMng->HighlightMeshGroup(AnimMng->GetAnimCompDefMesh(idx), groupnumber, true);
					}
				}
				delete[] sellist;
				}
		...
		case IDC_BUTTON_ANIMCOMPPASTEV:
		{
			if (VB1->vclip.valid) {
				...
				if (!AnimMng->IsAnimCompArmTip(idx))
				{
					msh_pos = VB1->MshMng->GetMeshDefPos(VB1->MshMng->IdxMsh2Def(AnimMng->GetAnimCompDefMesh(idx)));
				}
				SetDlgItemsTextVector3(hWnd, IDC_EDIT_ANIMCOMPREFX, IDC_EDIT_ANIMCOMPREFY, IDC_EDIT_ANIMCOMPREFZ, VB1->vclip.pos-msh_pos);
				SetDlgItemsTextVector3(hWnd, IDC_EDIT_ANIMCOMPAXISX, IDC_EDIT_ANIMCOMPAXISY, IDC_EDIT_ANIMCOMPAXISZ, VB1->vclip.dir);
		...
		case IDC_COMBO_ANIMCOMPMESH:
		{
			if (HIWORD(wParam) == CBN_SELCHANGE) {
				...
				AnimMng->ResetAnimCompDefGroups(idx);
				UpdateAnimCompGroupLists(hWnd, VB1->MshMng->IdxMsh2Def(AnimMng->GetAnimCompDefMesh(idx)));
				AnimMng->SetIsAnimCompArmTip(idx, false);
			}
			break;
		...
		case IDC_COMBO_ANIMCOMPPARENT:
		{
			if (HIWORD(wParam) == CBN_SELCHANGE) {
				...
				AnimMng->SetAnimCompDefParent(idx, parent_ach);
			}
